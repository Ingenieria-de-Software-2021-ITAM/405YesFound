...
// ---------------------------------------------------------------------------    
    public MiModelo obtenMaxId() {
        conexion.conectate();
        java.sql.ResultSet rs = conexion.obtenRegSelect("Select max(clvProyecto) from tblProyectos");
        MiModelo elModelo = new MiModelo(rs);
        conexion.desconectate();
        return elModelo;
    }
// ---------------------------------------------------------------------------    
    public MiModelo obtenVotos() {
        conexion.conectate();
        String query = "SELECT NOMBREPRO, COUNT(tblVoto.CLVPROYECTO) AS TOTAL FROM tblVoto RIGHT JOIN tblProyectos " +
                        "ON tblVoto.CLVPROYECTO = tblProyectos.CLVPROYECTO GROUP BY NOMBREPRO";
        java.sql.ResultSet rs = conexion.obtenRegSelect(query);
        MiModelo elModelo = new MiModelo(rs);
        conexion.desconectate();
        return elModelo;
    }
...
// ---------------------------------------------------------------------------
    ...
    public boolean altaProyecto(String arr_nomCampos[], String arr_datos[]) {
        conexion.conectate();
        int i, n;
        // clvAlumno, clvContiend, clvProyecto
        // se obtiene la colección de campos de la tabla de los alumnos
        java.util.TreeMap<String, ClsCampoBD> colCampos = conexion.obtenMapaCampos(conexion.obtenRS("tblProyectos"));
        n = arr_nomCampos.length;
        // se rellena el valor para cada campo
        for (i = 0; i < n; i++) {
            ((ClsCampoBD) colCampos.get(arr_nomCampos[i])).valor = arr_datos[i];
        ...
        // se solicita al objeto conexión que inserte el registro y se espera el resultado
        Boolean ans = conexion.insertaReg("tblProyectos", colCampos);
        conexion.desconectate();
        return ans;
    }
// ---------------------------------------------------------------------------    
